...
async function randomWordGen(counter, error = false, sync = true) {
  if (counter % 3 == 0 && counter % 5 != 0) return (counter + ": Fizz\n");
  else if (counter % 5 == 0 && counter % 3 != 0) return (counter + ": Buzz\n");
  else if (counter % 3 == 0 && counter % 5 == 0) return (counter + ": FizzBuzz\n");
  else {
    if (sync)
      return (counter + ": " + getRandomWordSync({ withErrors: error }) + "\n");
    else
      return (counter + ": " + await getRandomWord({ withErrors: error }) + "\n");
  };
}
